...
window.JuiceEngine = (() => {
  ...
  async function resolveAndPlayTrack(track) {
    ...
    // Trigger Auto-Gen Radio queue replenishment if near end of queue
    if (state.autoplay !== false && (state.currentIndex >= state.queue.length - 3 || state.queue.length <= 1)) {
      setTimeout(() => autoGenerateRadioQueue(track), 800);
    }
  ...
  // ═══ YOUTUBE MUSIC AUTO-GENERATED RADIO QUEUE & INFINITE AUTOPLAY ═══
  let isAutoGeneratingRadio = false;

  async function autoGenerateRadioQueue(seedTrack) {
    if (!seedTrack || isAutoGeneratingRadio) return [];
    isAutoGeneratingRadio = true;

    try {
      const artist = (seedTrack.artist || '').trim();
      const title = (seedTrack.title || '').trim();
      let relatedTracks = [];

      // 1. Check Pre-cached Catalogs for immediate 0ms related tracks
      if (typeof TOP_SHUFFLES_CATALOG !== 'undefined') {
        const matchingCatalog = Object.values(TOP_SHUFFLES_CATALOG).find(cat => {
          return cat.tracks && cat.tracks.some(t => 
            (t.artist && artist && t.artist.toLowerCase().includes(artist.toLowerCase())) ||
            (artist && t.artist && artist.toLowerCase().includes(t.artist.toLowerCase()))
          );
        });

        if (matchingCatalog && matchingCatalog.tracks) {
          relatedTracks.push(...matchingCatalog.tracks);
        }
      }

      // 2. Fetch Dynamic YouTube Music Radio Mix via API Search
      const searchQueries = [
        `${artist} similar songs official audio`,
        `${title} ${artist} radio mix`,
        `${artist} greatest hits audio`
      ];

      const chosenQuery = searchQueries[Math.floor(Math.random() * searchQueries.length)];
      try {
        const dynamicResults = await api.search(chosenQuery);
        if (dynamicResults && dynamicResults.length > 0) {
          relatedTracks.push(...dynamicResults);
        }
      } catch(e) {
        console.warn("[Auto-Gen Radio] Dynamic fetch fallback:", e);
      }

      // Filter duplicates already in queue or matching seedTrack
      const existingIds = new Set(state.queue.map(t => t.id || t.title));
      existingIds.add(seedTrack.id || seedTrack.title);

      const uniqueRelated = [];
      for (const t of relatedTracks) {
        const idKey = t.id || t.title;
        if (!existingIds.has(idKey) && t.title !== seedTrack.title) {
      ...
      // Shuffle related tracks for dynamic mix
      for (let i = uniqueRelated.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [uniqueRelated[i], uniqueRelated[j]] = [uniqueRelated[j], uniqueRelated[i]];
      }

      const topPicks = uniqueRelated.slice(0, 18);
      if (topPicks.length > 0) {
        state.queue.push(...topPicks);
        emit('engine:queueUpdated', state.queue);
        console.log(`[Auto-Gen Radio] Appended ${topPicks.length} related tracks for "${title}" by ${artist}`);
      }

      isAutoGeneratingRadio = false;
      return topPicks;
    } catch(err) {
      console.warn("[Auto-Gen Radio] Error generating queue:", err);
      isAutoGeneratingRadio = false;
      return [];
    }
  }
  ...
  const api = {
    autoGenerateRadio: (seedTrack) => autoGenerateRadioQueue(seedTrack),
    toggleAutoplay: () => {
